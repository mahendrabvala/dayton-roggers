...
namespace GKL.ApprovalEmails
{
    class Program
    {
                    ...
                    foreach (DataRow dr in dtCompanyIDs.Rows)
                    {
                        ...
                        try
                        {
                            ...
                            if (drDvisions.Length != 0)
                            {
                                ...
                                SendApproverEmail(companyID, divApprovalType, divEmailBuilder);
                        ...
                        try
                        {
                            ...
                            if (drAccounts.Length != 0)
                            {
                                ...
                                SendApproverEmail(companyID, accntApprovalType, accntEmailBuilder);
        ...
        private static void SendApproverEmail(string companyID, string approvalType, StringBuilder emailBody)
        {
        ...
        #region OldCode
        //private static void SendApproverEmail(ListItem headerItem, string body, string approvalType)
        //{
        //    //orglist
        //    CamlQuery camlOrgQuery = new CamlQuery();
        //    camlOrgQuery.ViewXml = "<View><Query><Where><Eq><FieldRef Name='Company_x0020_ID'/>" +
        //    "<Value Type='Text'>" + headerItem["Invoice_x0020_Company"].ToString() + "</Value></Eq></Where></Query><RowLimit>500</RowLimit></View>";
        //    ListItemCollection orgInfoCollection = listOrganizationInfo.GetItems(camlOrgQuery);
        //    context.Load(orgInfoCollection, items => items.Include(
        //             item => item.Id,
        //             item => item["Title"],
        ...
        //                        if (!string.IsNullOrEmpty(divApproverEmailID))
        //                        {
        //                            body = body.Replace("[UserName]", divisionalApprover.Title);
        //                            Utils.SendEmail(body, divApproverEmailID);
        //                        }
        //                    }
        //                }
        ...
        //                        if (!string.IsNullOrEmpty(accountingApproverEmailID))
        //                        {
        //                            body = body.Replace("[UserName]", accntApprover.Title);
        //                            Utils.SendEmail(body, accountingApproverEmailID);
        //                        }
        //                    }
        //                }
